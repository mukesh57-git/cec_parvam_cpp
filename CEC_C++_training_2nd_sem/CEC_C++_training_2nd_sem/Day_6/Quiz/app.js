/**
 * Day 6 Quiz App - Game Engine with Rooms and Persistence
 * Topic: Polymorphism in C++
 * Includes: Split-Screen Catch Game for First Turn
 */
...
const STORAGE_KEY = 'parvam_quiz_rooms_day6';
...
// MINI-GAME ENGINE (CATCH THE ORBS)
let minigame = {
  canvas: null,
  ctx: null,
  reqId: null,
  ...
  scoreB: 0,
  winScore: 5,
  paddleA: { x: 160, y: 330, width: 80, height: 12, speed: 8, color: '#a855f7' },
  paddleB: { x: 560, y: 330, width: 80, height: 12, speed: 8, color: '#ec4899' },
  balls: [],
  lastTime: 0,
  spawnTimer: 0,
  spawnInterval: 600,
  ...
  init() {
    ...
    // Key listeners only for minigame
  ...
  reset() {
    ...
    document.getElementById('btn-start-minigame').innerText = "Play Challenge";
  ...
  spawnBall() {
    ...
    const minX = isA ? 20 : 420;
    const maxX = isA ? 380 : 780;
    ...
    this.balls.push({
      ...
      y: -20,
      radius: 10,
      color: isA ? '#a855f7' : '#ec4899',
      side: isA ? 'A' : 'B',
      vx: (Math.random() - 0.5) * 5, // Erratic X movement
      vy: 3 + Math.random() * 3,     // Fast falling
  ...
  loop(timestamp) {
    ...
    if (this.scoreA >= this.winScore || this.scoreB >= this.winScore) {
      this.endGame();
    } else {
      this.reqId = requestAnimationFrame(t => this.loop(t));
    }
  ...
  update(dt, timestamp) {
    // Paddles
    if (this.keys['KeyA']) this.paddleA.x = Math.max(0, this.paddleA.x - this.paddleA.speed);
    if (this.keys['KeyD']) this.paddleA.x = Math.min(400 - this.paddleA.width, this.paddleA.x + this.paddleA.speed);
    
    if (this.keys['ArrowLeft']) this.paddleB.x = Math.max(400, this.paddleB.x - this.paddleB.speed);
    if (this.keys['ArrowRight']) this.paddleB.x = Math.min(800 - this.paddleB.width, this.paddleB.x + this.paddleB.speed);
    ...
    for (let i = this.balls.length - 1; i >= 0; i--) {
      let b = this.balls[i];
      // Erratic polymorphic movement
      b.x += b.vx + Math.sin(timestamp/150 + b.wobbleOffset) * 2;
      b.y += b.vy;

      // Bounce off walls (side A: 0-400, side B: 400-800)
      if (b.side === 'A') {
        if (b.x < b.radius) { b.x = b.radius; b.vx *= -1; }
        if (b.x > 400 - b.radius) { b.x = 400 - b.radius; b.vx *= -1; }
      } else {
        if (b.x < 400 + b.radius) { b.x = 400 + b.radius; b.vx *= -1; }
        if (b.x > 800 - b.radius) { b.x = 800 - b.radius; b.vx *= -1; }
      }

      // Check paddle collision
      let p = b.side === 'A' ? this.paddleA : this.paddleB;
      if (b.y + b.radius >= p.y && b.y - b.radius <= p.y + p.height &&
          b.x + b.radius >= p.x && b.x - b.radius <= p.x + p.width) {
          
          sfx.playTick(); // catch sound
          if (b.side === 'A') this.scoreA++; else this.scoreB++;
  ...
  draw() {
    ...
    this.ctx.fillText(this.scoreA, 200, 150);
    this.ctx.fillText(this.scoreB, 600, 150);
  ...
  endGame() {
    this.active = false;
    let winner = this.scoreA >= this.winScore ? 'A' : 'B';
    sfx.playSuccess();
    ...
    winText.id = 'win-text';
    winText.style.color = winner === 'A' ? this.paddleA.color : this.paddleB.color;
...
class ConfettiEngine {
  constructor(canvasId) {
    ...
    this.particles = [];
    this.active = false;
    this.colors = ['#f59e0b', '#a855f7', '#fbbf24', '#c084fc', '#fcd34d', '#e879f9'];
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  minigame.init(); // Initialize minigame
  initRoomsDatabase();
  initView();
  renderRoomsPanel();
});
